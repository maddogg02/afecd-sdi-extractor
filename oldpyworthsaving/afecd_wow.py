# --- helpers ---
def _next_all_caps_idx(lines, start_idx):
    for i in range(start_idx, len(lines)):
        if is_all_caps_title(lines[i]):
            return i
    return None
...
def parse_block(lines, pstart, pend, source_file):
    ...
    # find section starts (we’ll use 3 to bound duties and parse experience)
    ...
    # --- Section 2: Duties (bounded strictly by the start of Section 3 or next ALL-CAPS) ---
    if s2_pos is not None:
        start = idx_after_2
        stop  = q_pos if q_pos is not None else len(after)

        # also clip to next ALL-CAPS title within this window
        ac_rel = _next_all_caps_idx(after, start)
        if ac_rel is not None and ac_rel < stop:
            stop = ac_rel

        # build duties text from the bounded slice (no more Section 3 leakage)
        duties_slice = after[start:stop]
        duties_text  = norm_ws((" ".join(duties_slice)).strip())

        for code in fanout_codes:
            lvl = skill_from_code(code)
            if lvl == "00":    # don’t emit CEM into duties
                continue
            lvl_name  = skill_name_map.get(lvl, "")
            afsc_plain = code.rstrip("*")
            major2, family3 = major_from_afsc(code)
    ...
    # --- Section 3: Experience (AFSC only; skip for SDI) ---
    if q_pos is not None and not is_sdi_block:
        start = idx_after_3
        stop  = _next_all_caps_idx(after, start) or len(after)

        # normalize all of section 3 into a single string to tolerate inline "3.3. Experience."
        qual_text = norm_ws(" ".join(after[start:stop]))

        # locate the Experience subheader: 3.<N>. Experience.
        m_exp = re.search(r"\b3\.(\d+)\.\s*Experience\.", qual_text, flags=re.I)
        if m_exp:
            exp_number = m_exp.group(1)
            tail = qual_text[m_exp.end():]

            # extract items like: 3.<N>.<y>. <AFSC>. <rest...>  (non-greedy up to next 3.<N>.<y>. or end)
            ITEM_RE = re.compile(
                rf"3\.\d+\.(\d+)\.\s*({AFSC_RE.pattern})\.\s*(.*?)(?=3\.\d+\.\d+\.\s*|$)",
                re.I | re.S
            )

            for m in ITEM_RE.finditer(tail):
                subnum, afsc_in_item, rest = m.group(1), m.group(2), norm_ws(m.group(3))
                lvl = skill_from_code(afsc_in_item)
                if not lvl or lvl == "1":
                    continue  # ignore non-awardable levels
                ...
                rows.append({
                    ...
                    "id": make_id(afsc_plain, lvl, "experience", section_index, specialty_title, rest),
                })
